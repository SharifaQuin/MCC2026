    ...
    >
      <div className="flex items-center justify-between text-xs text-neutral-500">
        <span>
          ...
          {isInbound ? " — from lead" : entry.sentByName ? ` — ${entry.sentByName}` : ""}
        </span>
        ...
        <div>
          ...
          {tab === "logEmail" && (
            <form
              action={(formData) =>
                startTransition(async () => {
                  await logLeadReplyAction(leadId, "EMAIL", formData);
                  setTab("none");
                })
              }
            ...
            >
              <p className="text-xs text-neutral-500">
                Paste what the lead said in their email reply — this just records it here, it
        ...
        <div>
          ...
          {tab === "logSms" && (
            <form
              action={(formData) =>
                startTransition(async () => {
                  await logLeadReplyAction(leadId, "SMS", formData);
                  setTab("none");
                })
              }
              className="mb-2 space-y-2 rounded-md border border-neutral-200 bg-neutral-50 p-3"
            >
              <p className="text-xs text-neutral-500">
                Paste what the lead texted back — this just records it here, it doesn&apos;t send
                anything.
              </p>
              <textarea
                name="body"
